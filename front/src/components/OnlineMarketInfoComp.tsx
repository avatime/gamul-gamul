      ...
      <CardContainer title={"온라인"} style={{width}}> 
        <Table>
          {onlineMartInfo.map((row, idx) => (
            ...
            <TableRow key={idx} onClick= {()=>{window.open(row.url)}} sx={{cursor:"pointer"}} className={styles.hover}>
                    
                <TableCell>{row.name}</TableCell>
                <TableCell><Stack direction="row"  sx={{alignItems:"center"}}><Lottie animationData={cart} loop={true} autoPlay={false} style={{width:iconSize, height:iconSize}}/>{row.mall_name}</Stack></TableCell>
