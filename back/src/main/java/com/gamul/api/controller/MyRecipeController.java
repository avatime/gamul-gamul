        ...
        try {
            ...
            for (MyRecipeIngredient myRecipeIngredient : myRecipeIngredientList) {
                ...
                if (dailyPrice.size() > 0) {
                    ...
                    if (dailyPrice.size() > 1) {
                        volatility = (1.0 * (dailyPrice.get(0).getPrice() - dailyPrice.get(1).getPrice()) / dailyPrice.get(1).getPrice()) * 100;
                    ...
                    double volatility = 0.0;
                    if (dailyWholePrice.size() > 1) {
                        volatility = (1.0 * (dailyWholePrice.get(0).getPrice() - dailyWholePrice.get(1).getPrice()) / dailyWholePrice.get(1).getPrice()) * 100;
                        volatility = Math.round(volatility * 100) / 100.0;
                    }
                    MyRecipeIngredientInfoRes ingredientInfoRes = MyRecipeIngredientInfoRes.builder()
                            .ingredientId(myRecipeIngredient.getIngredient().getId())
                    ...
                    double volatility = 0.0;
                    if (monthlyPrice.size() > 1) {
                        volatility = (1.0 * (monthlyPrice.get(0).getPrice() - monthlyPrice.get(1).getPrice()) / monthlyPrice.get(1).getPrice()) * 100;
                        volatility = Math.round(volatility * 100) / 100.0;
                    }
                    MyRecipeIngredientInfoRes ingredientInfoRes = MyRecipeIngredientInfoRes.builder()
                            .ingredientId(myRecipeIngredient.getIngredient().getId())
                    ...
                    double volatility = 0.0;
                    if (monthlyWholePrice.size() > 1) {
                        volatility = (1.0 * (monthlyWholePrice.get(0).getPrice() - monthlyWholePrice.get(1).getPrice()) / monthlyWholePrice.get(1).getPrice()) * 100;
                        volatility = Math.round(volatility * 100) / 100.0;
                    }
                    MyRecipeIngredientInfoRes ingredientInfoRes = MyRecipeIngredientInfoRes.builder()
                            .ingredientId(myRecipeIngredient.getIngredient().getId())
                ...
                if (ingreidentlist.get(ingreidentlist.size() - 1).getPrice() != 0)
                    total_price += (ingreidentlist.get(ingreidentlist.size() - 1).getPrice() / ingreidentlist.get(ingreidentlist.size() - 1).getQuantity()) * myRecipeIngredient.getQuantity();
            }
